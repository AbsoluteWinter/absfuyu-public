...
from absfuyu.util.text_table import BoxDrawingCharacter, OneColumnTableMaker
...
class TestOneColumnTableMaker:
    ...
    def test_make_line_upper(self, table_maker: OneColumnTableMaker) -> None:
        ...
        assert line.startswith(BoxDrawingCharacter.UPPER_LEFT_CORNER)
        assert line.endswith(BoxDrawingCharacter.UPPER_RIGHT_CORNER)

    def test_make_line_intersect(self, table_maker: OneColumnTableMaker) -> None:
        line = table_maker._make_line(1)
        assert line.startswith(BoxDrawingCharacter.VERTICAL_RIGHT)
        assert line.endswith(BoxDrawingCharacter.VERTICAL_LEFT)

    def test_make_line_lower(self, table_maker: OneColumnTableMaker) -> None:
        line = table_maker._make_line(2)
        assert line.startswith(BoxDrawingCharacter.LOWER_LEFT_CORNER)
        assert line.endswith(BoxDrawingCharacter.LOWER_RIGHT_CORNER)
